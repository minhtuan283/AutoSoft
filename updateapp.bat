...
set "SOURCE_FILE_NAME=App_4.zip"
set "SOURCE_DOWNLOAD_URL=https://softupdate.tangtuanlab.io.vn/App_4.zip?v=999"
...
echo Dang tai file cap nhat: !SOURCE_ZIP_URL!
echo.

rem --- Primary: curl.exe (built-in Windows 10+) ---
curl.exe -L -k --retry 3 --connect-timeout 30 --max-time 7200 -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64)" -o "%TEMP_ZIP%" "!SOURCE_ZIP_URL!"
if %ERRORLEVEL% EQU 0 if exist "%TEMP_ZIP%" goto :download_ok

rem --- Fallback: bitsadmin (also built-in, no AV flags) ---
echo curl.exe that bai, thu lai bang bitsadmin...
bitsadmin /transfer "AutoSoft_AppUpdate" /download /priority FOREGROUND "!SOURCE_ZIP_URL!" "%TEMP_ZIP%" >nul
if %ERRORLEVEL% EQU 0 if exist "%TEMP_ZIP%" goto :download_ok

rem --- Last resort: PowerShell Invoke-WebRequest (plain, no Base64) ---
echo bitsadmin that bai, thu lai bang PowerShell...
powershell -NoProfile -ExecutionPolicy Bypass -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri '!SOURCE_ZIP_URL!' -OutFile '%TEMP_ZIP%' -UseBasicParsing -TimeoutSec 7200"
if %ERRORLEVEL% EQU 0 if exist "%TEMP_ZIP%" goto :download_ok

echo [LOI] Khong the tai file tu !SOURCE_ZIP_URL!
exit /b 1
